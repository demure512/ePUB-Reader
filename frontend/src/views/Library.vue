            ...
            >
              ...
              <div class="book-info">
                ...
                <div class="book-meta">
                  ...
                  <span class="book-category" v-if="book.category">{{
                    book.category
                  }}</span>
                </div>
                <div v-if="getBookProgress(book) > 0" class="progress-bar">
                  <div
                    ...
                    :style="{ width: getBookProgress(book) + '%' }"
                  ></div>
                </div>
              </div>
...
const deleteBook = async (id) => {
  try {
    await ElMessageBox.confirm("确定要删除这本书吗？", "删除确认", {
...
onMounted(async () => {
  ...
  if (bookStore.books.length > 0) {
    ...
    bookStore.books.forEach((book, index) => {
      ...
      console.log(
        `- 封面字段长度: ${book.coverImage ? book.coverImage.length : "N/A"}`
      );
      if (book.coverImage && book.coverImage.startsWith("data:")) {
        console.log(
          `- 封面格式: base64 (${book.coverImage.substring(0, 50)}...)`
        );
      }
