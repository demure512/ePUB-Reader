  ...
  <header class="reader-header">
    ...
    <div class="book-info">
      ...
      <p class="location-info">
        ...
        <span
          v-if="
            book.fileType === 'EPUB' && isMainContent && totalLocations === 0
          "
          >正在计算页码...</span
        >
        <span
          v-else-if="
            book.fileType === 'EPUB' && isMainContent && totalLocations > 0
          "
          >{{ currentLocation }} / {{ totalLocations }}</span
        >
...
const isMainContent = computed(() => {
  ...
  const excludeKeywords =
    /封面|cover|版权|copyright|前言|preface|序言|introduction|致谢|acknowledgment|about|info|publisher|出版|作者|author|title|目录|contents|toc/i;
...
const downloadBook = async () => {
  ...
  try {
    ...
    if (authStore.isGuest) {
      ...
      a.download =
        props.book.fileName ||
        `${props.book.title}.${props.book.fileType.toLowerCase()}`;
